...
async def load_information(msg : types.Message, state : FSMContext):
    ...
    try:
        ...
        try:
            ...
            await bot.send_message(msg.from_user.id, f"Данные успешно заполнены, подождите пока с вами не свяжутся", reply_markup=client_kb.client_kb1)
    except:
        await bot.send_message(msg.from_user.id, f"Произошла ошибка добавления в базу данных", reply_markup=client_kb.client_kb1)
